...
def convenient_account(balance, dependencies=[]):
  ...
  def own_dispatch(action, params=''):
    if action == 'report':
      return f'Your account balance is {balance} USD.'
    elif action == 'id':
      return 'convenient_account'
    else:
      return 'wat?'
  ...
  def dispatch(action, params=''):
    own_result = own_dispatch(action, params)
    if own_result != 'wat?':
      return own_result
    else:
      for dependency in dependencies:
        dep_at_hand = dependency(balance)
        dep_result = dep_at_hand(action, params)
        if dep_result != 'wat?':
          if callable(dep_result) and dep_result('id') == dep_at_hand('id'):
...
print(convenient_account(100, [savings_account, account])('deposit', 30)('balance'))
print(convenient_account(100, [savings_account, account])('deposit', 130)('report'))

def somewhat_universal_account_constructor(*args, dependencies=[]):

  def own_dispatch(action, *params):
    if action == 'id':
      return 'somewhat_universal_object_constructor'
    elif action == 'report':
      return f'You have {args[0]} on this account.'
    else:
      return 'wat?'
...
print(somewhat_universal_account_constructor(100, dependencies=[savings_account, account])('deposit', 80)('report'))
